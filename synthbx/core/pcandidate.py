...
from synthbx.xast.type import Type, NumberType, SymbolType
from synthbx.xast.relation import Relation, RejectRelation, ValidRelation
...
from synthbx.xast.atom import Atom, RejectAtom, ValidAtom
...
from synthbx.xast.constant import Constant, NullConstant
from synthbx.xenv.const import ESuffix, EPrefix
...
def build_tlr_s(arule, fexample, relation_decls):
    ...
    t_del[0] = [t.data for t in fexample.tables
                if t.name == source.name and t.is_deleted()][0]
 
    cs_vars = list({cs.var for cs in constraint})
    cons = Atom(name='temp', args=cs_vars)
    
    index_args_c = [source.args.index(a) for a in cs_vars]
    index_args_d = [source.args.index(a) for a in args_d(source, cons)]
    ...
    rplc_rows = []
    for row in t_ins[0]:
        pri = tuple([row[i] for i in index_args_d])
        if pri in proj_row_del:
            rplc_rows.append(row)
    
    if rplc_rows:
        fn = [
            f'{EPrefix.AUX}_S_0_{view.name}_{source.name}',
            f'{EPrefix.AUX}_S_1_{view.name}_{source.name}'
        ]
        ...
        cs_vars_repl = [Variable(name=f'{v.name}r') for v in cs_vars]
        rargs = args_ra(source, cs_vars, cs_vars_repl)
        
        prci = [tuple([row[i] for i in index_args_c]) for row in rplc_rows]

        if len(set(prci)) == 1:
            relation_decls.append(
                Relation(
                    name=fn[0],
                    schema=[s_schema[i] for i in index_args_c]
                ).clone()
            )
            
            f_args = [Constant(value=f'"{ri}"') for ri in prci[0]]
            fs.append(
                Fact(atom=Atom(
                    name=fn[0],
                    args=f_args
            ...
            rs_fg.append(
                Rule(
                    ...
                    body=[Conjunction(items=[
                        ...
                        Atom(
                            name=source.name + ESuffix.UPDATE,
                            args=args_jwa(source, sources[1-i])
                        )])
                    ]
                ).clone()
...
def args_ra(l, vs1, vs2):
    """_summary_

    Args:
        l (Atom): _description_
        vs1 (list): list<Variable>
        vs2 (list): list<Variable>

    Returns:
        list<Variable>
    """
    return [a if a not in vs1 else vs2[vs1.index(a)] for a in l.args] \
            if set(vs1).issubset(l.args) and len(vs1) == len(vs2) else []
