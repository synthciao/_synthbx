...
from synthbx.xast.rule import Rule, AtomicRule
...
from synthbx.xast.variable import Variable, AnonymousVariable
from synthbx.xast.constant import Constant
from synthbx.xenv.const import EPrefix, ESuffix
from synthbx.xenv.exception import NotSafeError, NotDecomposableError
...
def decompose(program):
    ...
    for rule in prog.rules:
        d1_rules, d1_relation_decls, d1_directives = d_ml2_x(
    ...
    d_ml3_u(d_rules, prog.relation_decls, prog.directives)

    d_ml4_id(d_rules, prog.relation_decls, prog.directives)
...
def decompose1(rule, relation_decls, directives):
    ...
    while True:
        d_rules_n[0], conj = d_sl1_s(
            head, conj, d_relation_decls, d_directives, env
        )
        if Rule(head, [conj]).is_atomic():
            n = 0
            break
        d_rules_n[1], conj = d_sl2_p(
            head, conj, d_relation_decls, d_directives, env
        )
        if Rule(head, [conj]).is_atomic():
            n = 1
            break
        d_rules_n[2], conj = d_sl3_ji(
            head, conj, d_relation_decls, d_directives, env
        )
        if Rule(head, [conj]).is_atomic():
            n = 2
            break
        d_rules_n[3], conj = d_sl4_p(
            head, conj, d_relation_decls, d_directives, env
        )
        if Rule(head, [conj]).is_atomic():
            n = 3
            break
        d_rules_n[4], conj = d_sl5_jd(
            head, conj, d_relation_decls, d_directives, env
        )
        if Rule(head, [conj]).is_atomic():
            n = 4
            break
        d_rules_n[5], conj = d_sl6_p(
            head, conj, d_relation_decls, d_directives, env
        )
        n = 5
        break
    ...
    d_sl_id(head, conj, d_rules, d_relation_decls, d_directives, env)
...
def d_ml2_x(rule, relation_decls, directives):
    return decompose1(rule, relation_decls, directives)


def d_sl1_s(head, conj, d_relation_decls, d_directives, env):
    global counter
...
def d_sl2_p(head, conj, d_relation_decls, d_directives, env):
    global counter
...
def d_sl3_ji(head, conj, d_relation_decls, d_directives, env):
    global counter
...
def d_sl4_p(head, conj, d_relation_decls, d_directives, env):
    global counter
...
def d_sl5_jd(head, conj, d_relation_decls, d_directives, env):
    global counter
...
def d_sl6_p(head, conj, d_relation_decls, d_directives, env):
    global counter
    ...
    l = pl_list[0]
    p = [v for v in l.args
         if v not in head.args and v not in [cs.var for cs in cs_list]
         ]
...
def d_sl_id(head, conj, d_rules, d_relation_decls, d_directives, env):
...
def d_ml3_u(d_rules, d_relation_decls, d_directives):
...
def d_ml4_id(d_rules, d_relation_decls, d_directives):
...
#============================
        ...
        data = '''
        .decl S1(a:symbol, b:symbol, c:symbol, e:symbol)
        .decl S2(a:symbol)
        .decl S3(a:symbol, c:symbol)
        .decl S4(b:symbol)
        .decl S5(a:symbol, d:symbol)
        .decl V(a:symbol, d:symbol)

        .input S1, S2, S3, S4, S5
        .output V
        
        V(a,d) :- S1(a,b,c,e), S2(a), S3(a,c), !S4(b), b="T", d="0"; S2(a), S5(a,d).
        '''
